	...
	, mGUIContainer()
{
	...
	for (std::size_t x = 0; x < 2; ++x)
	{
		...
		addButtonLabel(PlayerAction::Jump, x, 2, "Jump", context);
		addButtonLabel(PlayerAction::Fire, x, 3, "Fire", context);
		addButtonLabel(PlayerAction::LaunchMissile, x, 4, "Missile", context);
	}

	updateLabels();

	auto backButton = std::make_shared<GUI::Button>(context);
	backButton->setPosition(420.f, 620.f);
...
void SettingsState::addButtonLabel(std::size_t index, std::size_t x, std::size_t y, const std::string& text, Context context)
{
	...
	mBindingButtons[index]->setPosition(400.f*x + 215.f, 50.f*y + 300.f);
	...
	mBindingLabels[index]->setPosition(400.f*x + 430.f, 50.f*y + 315.f);
